    ...
    export default {
        ...
        methods: {
            ...
            async fetchItems(page) {
                ...
                /*
                let query = this.config.tableName!=undefined?gql`${commGql.itemsGql}`:gql`${that.config.itemsGql}`;
                console.log(query);
                //let query = gql`${this.config.itemsGql}`;
                this.$apollo.query({
                    query: query,
                    variables: {
                        token:globalConfig.token,
                        tableName:this.config.tableName,
                        where:JSON.stringify(search),
                    ...
                    that.pagination.total = respData.count;
                    let datas = respData[queryKey];
                    that.items=[];
                    if(this.config.tableName!=undefined){
                        for(let i in datas )
                            that.items.push(JSON.parse(datas[i]));
                    }else{
                        for(let i in datas )
                            that.items.push(datas[i]);
                    }
